...
pub unsafe fn exp(a: __m128, b: __m128) -> (__m128, __m128) {
    ...
    let mut uv: [f32; 2] = core::mem::uninitialized();
...
pub unsafe fn log(p1: __m128, p2: __m128) -> (__m128, __m128) {
    ...
    // from the motor.
    let bv_mask = _mm_set_ps(1.0, 1.0, 1.0, 0.0);
    ...
    let mut p = core::mem::uninitialized();
    _mm_store_ss(&mut p, p1);

    // Store the pseudoscalar component
    let mut q = core::mem::uninitialized();
    _mm_store_ss(&mut q, p2);

    let mut s_scalar = core::mem::uninitialized();
    _mm_store_ss(&mut s_scalar, s);
    let mut t_scalar = core::mem::uninitialized();
    ...
    let p_zero = p.abs() < 1e-6;
    let u = if p_zero {
        f32::atan2(-q, t_scalar)
    } else {
        f32::atan2(s_scalar, p)
    };
